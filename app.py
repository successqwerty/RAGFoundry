...
with st.form(key="query_composer_form", clear_on_submit=False):
    ...
    with col_comp_left:
        st.markdown(f"""
            <div style="font-size: 12px; color: {T['text_sec']}; margin-top: 8px;">
...
# 10. SUGGESTED QUERY CARDS (Theme-Aware Cards, NO Solid Purple Bars)
...
# 13. CONVERSATION, GROUNDED ANSWER & EVIDENCE WORKSPACE
if "last_result" in st.session_state:
    ...
    st.markdown(f"""
        <div class="rf-user-card">
            ...
            <div style="font-size: 15px; font-weight: 500; color: {T['text_main']};">{res['question']}</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # EVIDENCE SECTION
    st.markdown(f'<h3 style="font-size: 16px; font-weight: 600; margin-top: 24px; margin-bottom: 12px; color: {T["text_main"]};">Evidence</h3>', unsafe_allow_html=True)
